...
//
// Deprecated: use subtitle.NewXMLToSRTConverter instead.
func ConvertXMLToSRT(xmlContent []byte) (string, error) {
	converted, err := subtitle.NewXMLToSRTConverter().Convert(xmlContent)
	return string(converted), err
}

// ConvertXMLFileToSRT converts XML subtitles file to SRT format.
//
// Deprecated: use subtitle.Processor.ConvertFile instead.
func ConvertXMLFileToSRT(xmlPath string) (string, error) {
	return subtitle.New().ConvertFile(xmlPath)
}
