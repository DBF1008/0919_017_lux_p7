...
func ConvertXMLToSRT(xmlContent []byte) (string, error) {
	srtContent, err := subtitle.ConvertXMLToSRT(xmlContent)
	if err != nil {
		return "", err
	}
...
func ConvertXMLFileToSRT(xmlPath string) (string, error) {
	converter, ok := subtitle.ConverterFor(filepath.Ext(xmlPath))
	if !ok {
		return "", errors.Errorf("no subtitle converter registered for %s", xmlPath)
	}
	return subtitle.ConvertFile(xmlPath, converter)
}
